...
{
    const char *disableViewer = std::getenv("ORB_SLAM2_DISABLE_PCL_VIEWER");
    const bool showViewer = !(disableViewer && std::string(disableViewer) == "1");
    std::unique_ptr<pcl::visualization::CloudViewer> viewer;
    if (showViewer)
    {
        viewer.reset(new pcl::visualization::CloudViewer("PointCloud Viewer"));
    }
